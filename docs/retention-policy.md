...
2. **hard-delete**: بعد 7 أيام من الوسم يُتلف فعلياً — ملف الصوت يُحذف من القرص،
   وصف التفريغ يُحذف من القاعدة.

> **دقة التنفيذ (مراجَعة 2026-08-03):** مرحلة الوسم (soft) تسري على الأثرين
> اللذين يملكان عمود `deleted_at`: **الصوت** و**التفريغ**. أما `edit_events`
> و`processing_attempts` و`note_versions` المنقولة و`audit_logs` فتُحذف صلباً
> مباشرةً **بعد مدتها مضافاً إليها فترة السماح نفسها** (المدة + 7 أيام) — لا
> مرحلة وسم لها لأنها صفوف سجلّية لا محتوى قابل للاسترجاع التشغيلي. النتيجة
> الزمنية واحدة: لا يُتلف أثر قبل انقضاء مدته + 7 أيام.
>
> **صفوف `recordings` لا تُحذف أبداً** — الملف يُتلف والصف يبقى أثراً مختوماً
> بـ`deleted_at` (`REVOKE DELETE` قائم من هجرة 0001).
