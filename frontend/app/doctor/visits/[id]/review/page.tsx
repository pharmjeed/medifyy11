...
const SECTION_TITLES: Record<string, string> = {
  ...
  H: "التاريخ المرضي — History",
};

const KIND_META: Record<GuidanceItem["kind"], { label: string; bg: string; fg: string }> = {
  coding_match: { label: "ترميزي", bg: "rgba(42,111,151,.12)", fg: "#2A6F97" },
  clinical_dx: { label: "سريري — تشخيص", bg: "#EAF6F7", fg: "#0A5C64" },
  clinical_rx: { label: "سريري — دواء", bg: "#EAF6F7", fg: "#0A5C64" },
  clinical_procedure: { label: "سريري — إجراء", bg: "#EAF6F7", fg: "#0A5C64" },
};

const STATUS_META: Record<GuidanceItem["status"], { label: string; bg: string; fg: string }> = {
  pending: { label: "معلق", bg: "#FDF3E3", fg: "#B07D10" },
  accepted: { label: "مقبول", bg: "#E8F6EE", fg: "#2E9E5B" },
  rejected: { label: "مرفوض", bg: "#FDEEEE", fg: "#C0392B" },
  modified: { label: "مقبول — معدّل", bg: "#E8F6EE", fg: "#2E9E5B" },
};
...
export default function ReviewPage() {
  ...
  const saveTyping = async (section: SummarySection) => {
    try {
      ...
      setEditing(null);
      toast("حُفظت الفقرة — edit_event(typing) (FR-705)");
      void load();
    } catch (err) {
  ...
  const stopDictation = async (merge: boolean) => {
    ...
    try {
      ...
      setEtag(result.data.etag);
      toast("دُمج الإملاء في الفقرة المحددة — edit_event(voice) (FR-706)");
      void load();
    } catch (err) {
  ...
  const resolveGuidance = async (item: GuidanceItem, status: "accepted" | "rejected" | "modified") => {
    try {
      ...
      if (status === "accepted") toast("قُبل الإرشاد بفعل صريح (FR-704)");
      else if (status === "rejected") toast("رُفض الإرشاد — يبقى القرار مسجلاً");
      else toast("حُفظ الإرشاد معدلاً — النص والرمز معاً — وقُبل (FR-704)");
      setModifying(null);
      void load();
    } catch (err) {
  ...
  const undoPatch = async (messageIndex: number, patchIndex: number) => {
    ...
    try {
      ...
      toast("تُرجع عن التعديل — قابل للتراجع قبل الاعتماد فقط");
      void load();
    } catch (err) {
  ...
  const approve = async () => {
    ...
    if (counters.pending > 0) {
      toast(`لا يمكن الاعتماد — ${counters.pending} إرشادات معلقة (MDF-4222)`);
    ...
    if (withPendingText !== undefined) {
      toast(`عنصر معلق [ ] في قسم ${withPendingText.section_key} — عالجه قبل الاعتماد`);
  ...
  if (summary === null) {
    return (
      <Shell title="مساحة المراجعة الموحدة">
        <main className="page-wrap journey"><div className="grid-empty">جارٍ التحميل…</div></main>
      </Shell>
    );
  }
  ...
  return (
    <Shell title="مساحة المراجعة الموحدة">
      ...
      <main className="page-wrap journey" style={{ paddingBottom: 150 }}>
        <SpecBar ids="W-214 · W-215 · W-216 · W-217 · W-218 · W-219 · W-220 · W-222 · W-224" desc="الصفحة 15 — الصفحة المحورية" />

        {/* رأس الزيارة */}
        <div className="card" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <strong style={{ fontSize: 16 }}>مراجعة الزيارة</strong>
          <bdi className="tech-badge">{visitId.slice(0, 8)}</bdi>
          <span style={{ fontSize: 12.5, color: "#5B7280" }}>وُلّد بـ <bdi>{summary.model_ref}</bdi></span>
          <span style={{ flex: 1 }} />
          <span className="badge warn">معلق <span className="num">{counters.pending}</span></span>
          <span className="badge success">مقبول <span className="num">{counters.accepted}</span></span>
          <span className="badge danger">مرفوض <span className="num">{counters.rejected}</span></span>
          <button className="btn-row" onClick={() => void openTranscript()}>نص المحادثة الكامل</button>
          {locked ? (
            <span className="badge success">🔒 معتمدة — قراءة فقط (MDF-4226)</span>
          ) : null}
        </div>
        ...
        {analysisFailed ? (
          <div style={{ border: "2px solid #B07D10", background: "#FDF3E3", borderRadius: 12, padding: "12px 16px", marginTop: 12, display: "flex", gap: 10, alignItems: "center" }}>
            <SpecBadge id="W-224" />
            <strong style={{ color: "#B07D10" }}>فشل التحليل الذكي (<bdi>MDF-5033</bdi>)</strong>
            <span style={{ fontSize: 12.5, color: "#5B7280" }}>الملخص متاح بلا إرشادات — المراجعة والاعتماد متاحان، وسجّلنا إشعاراً بذلك.</span>
          </div>
        ) : null}
          ...
          return (
            <section key={section.id} id={`section-${section.id}`} className="card" style={{ marginTop: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                {section.is_edited ? <span className="badge info">معدّل</span> : null}
                {pendingText && !locked ? (
                  <button className="badge warn" style={{ border: "none", cursor: "pointer", animation: "mPulseW 2.2s ease infinite" }}
                    title="نص بين قوسين معلقين لم يُحسم — عالجه قبل الاعتماد"
                ...
                {!locked && editing !== section.id && dictating !== section.id ? (
                  <span style={{ display: "inline-flex", gap: 6 }}>
                    <button className="btn-row" onClick={() => { setEditing(section.id); setEditText(section.content_current); }}>✏ كتابة</button>
                    <button className="btn-row" onClick={() => startDictation(section)}>🎤 إملاء صوتي</button>
                    <button className="btn-row" onClick={() => document.getElementById("ai-chat")?.scrollIntoView({ behavior: "smooth" })}>💬 محادثة AI</button>
                  </span>
                ) : null}
              </div>
              ...
              {editing === section.id ? (
                <div style={{ border: "1.5px solid #0E7C86", borderRadius: 10, padding: 12, marginTop: 10 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: "#0A5C64", marginBottom: 6 }}>
                    وضع التحرير الكتابي <SpecBadge id="W-215" /> — يسجّل <bdi>edit_event(typing)</bdi>
                  </div>
                  <textarea className="field clinical" rows={5} value={editText} dir="ltr"
                    onChange={(event) => setEditText(event.target.value)} />
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <button className="btn-success" style={{ height: 38 }} onClick={() => void saveTyping(section)}>حفظ الفقرة</button>
                    <button className="btn-neutral" style={{ height: 38 }} onClick={() => setEditing(null)}>إلغاء</button>
                  </div>
                </div>
                ...
                <div style={{ marginTop: 10 }}>
                  ...
                  <div style={{ background: "#EAF6F7", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <span style={{ fontSize: 14 }}>جارٍ الاستماع للإملاء… تحدّث الآن</span>
                    ...
                    <span style={{ flex: 1 }} />
                    <button className="btn-success" style={{ height: 36 }} onClick={() => void stopDictation(true)}>إيقاف ودمج النص</button>
                    <button className="btn-neutral" style={{ height: 36 }} onClick={() => void stopDictation(false)}>إلغاء</button>
                  </div>
                  <p style={{ fontSize: 12.5, color: "#5B7280", margin: "6px 0 0" }}>
                    <SpecBadge id="W-216" /> مسار قصير غير متدفق بنموذج P1 نفسه — يُدمج في الفقرة المحددة (FR-706).
                  </p>
                </div>
              ) : (
              ...
              {section.guidance.length > 0 ? (
                <div style={{ borderTop: "1px dashed #D7E3E8", marginTop: 12, paddingTop: 10 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: "#5B7280" }}>
                    إرشادات هذه الفقرة (<span className="num">{section.guidance.length}</span>) — مضمّنة على الفقرة التي تخصها فقط
                  </div>
                    ...
                    return (
                      <div key={item.id} className={item.status === "pending" ? "guidance-card pending" : "guidance-card"}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                          ...
                          <span className="badge" style={{ background: kind.bg, color: kind.fg }}>{kind.label}</span>
                          {item.safety_flag && item.status === "pending" ? (
                            <span className="badge safety">⚠ سلامة مريض — بانتظار الحسم</span>
                          ) : null}
                          <span style={{ flex: 1 }} />
                        ...
                        <div style={{ fontSize: 12.5, color: "#5B7280", display: "flex", flexDirection: "column", gap: 2 }}>
                          <span>🕐 المصدر: {item.evidence_source === "patient_file" ? "من ملف المريض" : "من كلام الزيارة"} — {item.evidence_ref ?? "—"}</span>
                        </div>
                        {modifying?.id === item.id ? (
                          <div style={{ marginTop: 8 }}>
                            {item.code_system !== null ? (
                              <>
                                <label className="field-label" style={{ fontSize: 12.5 }}>الرمز — {item.code_system} (يُتحقق منه مقابل النظام النشط قبل الرفع)</label>
                                <input className="field mono" value={modCode} onChange={(event) => setModCode(event.target.value)} />
                              </>
                            ) : null}
                            <label className="field-label" style={{ fontSize: 12.5 }}>نص الإرشاد</label>
                            <textarea className="field clinical" rows={3} dir="ltr" value={modText} onChange={(event) => setModText(event.target.value)} />
                            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                              <button className="btn-success" style={{ height: 36 }} onClick={() => void resolveGuidance(item, "modified")}>حفظ وقبول معدلاً</button>
                              <button className="btn-neutral" style={{ height: 36 }} onClick={() => setModifying(null)}>إلغاء</button>
                            </div>
                          </div>
                        ) : item.status === "pending" && !locked ? (
                          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                            <button className="btn-success" style={{ height: 36 }} onClick={() => void resolveGuidance(item, "accepted")}>قبول</button>
                            <button className="btn-danger-outline" style={{ height: 36 }} onClick={() => void resolveGuidance(item, "rejected")}>رفض</button>
                            <button className="btn-row" onClick={() => { setModifying(item); setModText(item.suggestion_text); setModCode(item.code_value ?? ""); }}>تعديل</button>
                          </div>
                        ) : null}
                      </div>
                    );
        ...
        <section id="ai-chat" className="card" style={{ marginTop: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <strong style={{ fontSize: 16, flex: 1 }}>محادثة AI الختامية</strong>
            <SpecBadge id="W-217" />
          </div>
          <p style={{ fontSize: 12.5, color: "#5B7280", margin: "4px 0 10px" }}>
            تعدّل الملخص فقط — لا وقائع سريرية جديدة، والغموض سؤال توضيحي (DOC-15 §٥).
          </p>
          <div ref={chatRef} style={{ maxHeight: 340, overflowY: "auto", display: "flex", flexDirection: "column" }}>
                ...
                {message.patches?.map((patch, patchIndex) => (
                  <div key={patch.section_id + String(patchIndex)} style={{ border: "1px solid #D7E3E8", borderRadius: 10, padding: 10, marginBottom: 10, maxWidth: "88%" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, fontWeight: 700 }}>
                      بطاقة فرق — قسم <bdi>{patch.section_key}</bdi>
                      <span style={{ color: "#5B7280", fontWeight: 400 }}>تعليم ما تغيّر (FR-707)</span>
                      <span style={{ flex: 1 }} />
                      {message.undone?.[patchIndex] === true ? (
                        <span className="badge neutral">تم التراجع</span>
                      ) : !locked ? (
                        <button className="btn-danger-outline" style={{ height: 30, padding: "0 12px", fontSize: 12.5 }}
                          onClick={() => void undoPatch(messageIndex, patchIndex)}>تراجع</button>
                      ) : null}
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
                      <div style={{ background: "#FDEEEE", borderRadius: 8, padding: 8 }}>
                        <div style={{ fontSize: 12.5, fontWeight: 700, color: "#C0392B" }}>قبل</div>
                        <div className="clinical" style={{ color: "#C0392B", textDecoration: "line-through", fontSize: 12.5 }}>
                          {patch.old_content.length > 0 ? patch.old_content : "— (لم يكن موجوداً)"}
                        </div>
                      </div>
                      <div style={{ background: "#E8F6EE", borderRadius: 8, padding: 8 }}>
                        <div style={{ fontSize: 12.5, fontWeight: 700, color: "#2E9E5B" }}>بعد</div>
            ...
            {chatBusy ? <div className="chat-msg ai"><span className="spinner dark" /> يعالج طلبك…</div> : null}
          </div>
          {!locked ? (
            <>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "10px 0" }}>
                <button className="pill" onClick={() => void sendChat("اجعل المتابعة بعد أسبوع بدل أسبوعين")}>اجعل المتابعة بعد أسبوع بدل أسبوعين</button>
                <button className="pill" onClick={() => void sendChat("أضف إلى الخطة قياس الضغط المنزلي مرتين يومياً مع تسجيل القراءات.")}>أضف قياس الضغط المنزلي للخطة</button>
              </div>
              <form style={{ display: "flex", gap: 8 }} onSubmit={(event) => { event.preventDefault(); void sendChat(chatInput); }}>
                <input className="field" placeholder="اكتب أي طلب تعديل على الملخص…" value={chatInput}
                  onChange={(event) => setChatInput(event.target.value)} style={{ flex: 1 }} />
                <button type="submit" className="btn" disabled={chatBusy}>إرسال</button>
              </form>
            </>
          ) : null}
          <p style={{ fontSize: 12.5, color: "#5B7280", margin: "10px 0 0" }}>
            كل تعديل يسجّل <bdi>edit_event</bdi> بقناته — كتابة / صوت / محادثة (DOC-04 §٥).
          </p>
        </section>
      </main>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "12px 20px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          ...
          {upload.phase === "idle" && !locked ? (
            <>
              <span style={{ fontSize: 12.5, color: counters.pending === 0 ? "#2E9E5B" : "#B07D10", fontWeight: 700, flex: 1 }}>
                {counters.pending === 0
                  ? "صفر إرشادات معلقة — جاهزة للاعتماد · بالاعتماد تُرفع بيانات الزيارة كاملة بصيغة FHIR/NPHIES (FR-802)"
                  : `${counters.pending} إرشادات معلقة — الاعتماد لا يتفعل إلا بصفر معلق (MDF-4222)`}
              </span>
          ...
          {upload.phase === "uploading" ? (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 10, flex: 1 }}>
              <span className="spinner dark" /> جارٍ الرفع لنظام المستشفى — <bdi>Bundle FHIR/NPHIES</bdi>…
            </span>
          ) : null}
          {upload.phase === "done" ? (
            upload.status.status === "confirmed" ? (
              <span style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, flexWrap: "wrap" }}>
                <span className="badge success">رفع ناجح ✓ مؤكد confirmed</span>
                <span style={{ fontSize: 12.5, color: "#5B7280" }}>
                  المحاولات: <span className="num">{upload.status.attempts_count}</span> · التحقق البنيوي NPHIES: مجتاز ✓
                </span>
                <span style={{ flex: 1 }} />
                <button className="btn-secondary" onClick={() => router.push("/doctor/visits")}>سجل الزيارات</button>
              </span>
            ) : (
              <span style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, flexWrap: "wrap" }}>
                <span className="badge danger">فشل الرفع</span>
                <span style={{ fontSize: 12.5, color: "#5B7280" }}>
                  <bdi>{upload.status.attempts[upload.status.attempts.length - 1]?.error_code ?? "MDF-5052"}</bdi>
                  · المحاولات: <span className="num">{upload.status.attempts_count}</span> — أُشعر الأدمن (ad.upload_failed)
                </span>
                <span style={{ flex: 1 }} />
                <button className="btn" onClick={() => void retryUpload()}>إعادة المحاولة</button>
              </span>
            )
          ) : null}
        </div>
        ...
        <>
          ...
          <div className="drawer">
            <div style={{ padding: "14px 16px", borderBottom: "1px solid #D7E3E8", display: "flex", alignItems: "center", gap: 8 }}>
              <strong style={{ flex: 1 }}>نص المحادثة الكامل</strong>
              <SpecBadge id="W-220" />
              <button className="modal-close" aria-label="إغلاق" onClick={() => setTranscriptOpen(false)}>✕</button>
            </div>
            <div style={{ padding: "6px 16px", fontSize: 12.5, color: "#5B7280", borderBottom: "1px solid #EAF6F7" }}>
              محفوظ مرتبطاً بالزيارة (FR-604) · الصوت يُحذف آلياً وفق سياسة الاحتفاظ
            </div>
      ...
      {/* تعارض ETag W-222 */}
      {conflict !== null ? (
        <Modal title="تعارض تحرير — نسخة أحدث موجودة (MDF-4224)" spec="W-222" onClose={() => setConflict(null)} wide>
          <p style={{ fontSize: 14, color: "#5B7280" }}>عُدّل هذا الملخص من جلسة أخرى. قارن النسختين واختر:</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="sub-box">
              <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}>نسختك (غير المحفوظة)</div>
              <div className="clinical" style={{ fontSize: 12.5 }}>{conflict.mine}</div>
            </div>
            <div className="sub-box" style={{ borderColor: "#0E7C86" }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}>نسخة الخادم (الأحدث)</div>
          ...
          <div className="modal-actions">
            <button className="btn" onClick={() => { setConflict(null); void load(); toast("اعتُمدت نسخة الخادم"); }}>
              اعتماد نسخة الخادم
            </button>
            <button className="btn-secondary" onClick={async () => {
              ...
              try {
                ...
                setEtag(result.data.etag);
                toast("كُتبت نسختك فوق نسخة الخادم");
              } catch (err) {
                handleMutationError(err);
              }
              setConflict(null);
              void load();
            }}>الإبقاء على نسختي</button>
          </div>
        </Modal>
      ) : null}
    </Shell>
