      ...
      <ErrorScreenProvider>
        <header className="topbar">
          <div className="topbar-inner">
            <Link href="/sa" aria-label={L("ميديفاي — الرئيسية", "Medify — home")}><Logo /></Link>
            ...
            <span className="user-chip">
              <span style={{ textAlign: "start" }}>
                ...
                <span style={{ display: "block", fontSize: 12.5, color: "#5c7096", lineHeight: 1.3 }}>
                  {L(ROLE_LABEL[admin.role]?.ar ?? admin.role, ROLE_LABEL[admin.role]?.en ?? admin.role)}
                </span>
              </span>
              <span className="avatar" style={{ background: "#00c2b8", color: "#0c1a36" }}>{initials(admin.full_name)}</span>
            </span>
          ...
          <nav style={{ borderTop: "1px solid #d6f5f2", background: "#fff" }} aria-label={L("تنقل المنصة", "Platform navigation")}>
              ...
              {SA_NAV.filter((item) => item.cap === undefined || saCan(admin, item.cap)).map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} style={{
                    ...
                    borderBottom: active ? "3px solid #00c2b8" : "3px solid transparent",
                    whiteSpace: "nowrap",
        ...
        {!admin.totp_enabled && pathname !== "/sa/security" ? (
          <div style={{ background: "#fdf4e0", borderBottom: "1px solid #8fe0da", padding: "8px 20px", textAlign: "center", fontSize: 13.5 }}>
            ...
            <Link href="/sa/security" style={{ fontWeight: 700, color: "#9c6f00" }}>{L("فعّلها الآن ←", "Enable it now →")}</Link>
          </div>
        ) : null}
        {children}
      </ErrorScreenProvider>
