...
## الباقات: تسعير ومميزات (قرار مالك 2026-08-03 — تعديل معتمد على DOC-04/05/09/13/20)

- **يعدّل «لا باقات ميزات»** (DOC-20 تعديل ٢): الفاتورة تبقى عدد الدكاترة × تكلفة الدكتور،
  لكن **التكلفة صارت حسب نوع الباقة**، وكل باقة تحمل **ما تُظهره للدكتور** — كلاهما من `/sa/plans`.
- **الباقة صف واحد بسعرين** (هجرة 0022): `seat_price_sar` شهري + `seat_price_yearly_sar` سنوي
  (NULL = لا تُباع بتلك الدورة، وقيد CHECK يمنع باقة بلا سعر)، وخريطة مميزات **واحدة** لا تتفرّق
  بين شهري وسنوي — باقة بصفّين تعني تفعيل ميزة في أحدهما ونسيانها في الآخر، خطأ صامت عند العميل.
- **الدورة خاصية اشتراك لا باقة**: `subscriptions.billing_cycle`. الترحيل دمج الزوج المبذور
  `monthly`+`yearly` في باقة `standard` «قياسية» بسعريها، ونقل الاشتراكات إليها بدورتها المحفوظة.
- **الفوترة**: `billing.plan_seat_price(db, code, cycle)` + فترة الفاتورة تتبع الدورة
  (30 يوماً / 365) — اشتراك سنوي بفترة 30 يوماً كان خطأً كامناً قبل 0022.
- **صفحة التسجيل** تعرض الدورة ثم الباقات بسعرها و«ماذا تشمل» من `GET /plans` — قائمة المميزات
  من مصدر الإنفاذ نفسه لا نص تسويقي يفترق عنه.
